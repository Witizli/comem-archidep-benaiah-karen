...
define('BASE_URL', getenv('TODOLIST_BASE_ULR' ?: '/'));

// Database connection parameters.
define('DB_USER', getenv('TODOLIST_DB_USER') ?: 'todolist');
define('DB_PASS', getenv('TODOLIST_DB_PASS'));
define('DB_NAME', getenv('TODOLIST_DB_HOST') ?: 'todolist');
...
if (isset($_POST['action'])) {
  switch ($_POST['action']) {
    ...
    case 'toggle':
      ...
      $id = $_POST['id'];
      if (is_numeric($id)) {
        $updateQuery = 'UPDATE todo SET done = IF(done=1, 0, 1) WHERE id =' . $id; // SQL permettant de changer la couleur de la tâche
    ...
    case 'delete':
      ...
      $id = $_POST['id'];
      if (is_numeric($id)) {
        $deleteQuery = 'DELETE FROM todo WHERE id = ' . $id; // SQL permettant de supprimer la tâche
